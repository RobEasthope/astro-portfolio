---
employer: "?What If! Innovation"
employerUrl: "https://whatifinnovation.com/"
jobTitle: "WordPress developer"
jobType: "contract"
startDate: "2016-09-07"
endDate: "2016-10-21"
---

- Fully rebuilt a design consultancy agency site using responsive design methods
- Initial work was done in Squarespace, but the clients goals changed mid-way
- Pivoted to using WordPress, a custom theme, and extensive use of Advanced Custom Fields
